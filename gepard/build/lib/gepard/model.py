...
class NeuralModel(Model):
    ...
    def __init__(self, output_layer=['ReH', 'ImH'], **kwargs) -> None:
        ...
        self.useDR = kwargs.setdefault('useDR', None)
    ...
    def cffs(self, pt):
        #print("In cff")
        self.curname = self.output_layer[self.cff_index]
        if not self.in_training and not self.cffs_evaluated:
            ...
            if self.curname == 'ReH' and hasattr(self, 'useDR') and self.useDR:
                print("using DR")
                return cff.DispersionCFF.ReH(pt)
            if self.curname == 'ReE' and hasattr(self, 'useDR') and self.useDR:
                return cff.DispersionCFF.ReE(pt)

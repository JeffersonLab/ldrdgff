...
# Define file paths
RESULTS_DIR = '/Users/higuera-admin/Documents/Programs/ldrdgff/Analysis/tests/Results/DR4CFF_CLAS+HERMES'
FITS_DIR = '/Users/higuera-admin/Documents/Programs/ldrdgff/Analysis/tests/fits_models/DR4CFF_CLAS+HERMES'
...
class CustomNetwork(nn.Module):
	def __init__(self):
		...
		self.n1 = nn.Linear(2, 7)  # 2 input features, 20 hidden units
		self.n2 = nn.Linear(7, 5) # 20 hidden units, 25 hidden units
		self.n3 = nn.Linear(5, 2)  # 25 hidden units, 2 output units

		self.n1p = nn.Linear(1, 3)  # 1 input feature, 7 hidden units
		self.n2p = nn.Linear(3, 2)  # 7 hidden units, 5 hidden units
		self.n3p = nn.Linear(2, 1)  # 5 hidden units, 1 output unit
...
def train_one(i):
	...
	f = g.fitter.NeuralFitter(fitpoints, th, nnets=10, nbatch=10, batchlen=2, regularization='L2', lx_lambda=0.0001)
	f.fit()
	...
	}, os.path.join(FITS_DIR, f'nets_4CFFDR_{i+4}.pt'))
...
if __name__ == "__main__":
	...
	for i in range(ensembleSize):
		...
		plt.xlabel("Epochs")
		plt.ylabel("Loss")
		plt.title(f"Loss vs. Epochs (Model {i+1+4})")
		plt.legend()
		plt.grid()
		plt.savefig(os.path.join(RESULTS_DIR, f"loss4CFF_DRmodel_{i+1+4}.png"))
		plt.close()    
